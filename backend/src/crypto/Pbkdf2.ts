// PBKDF2-HMAC-SHA256 implemented from scratch — RFC 2898
...
export function pbkdf2(
  ...
  for (let blockIndex = 1; blockIndex <= blocksNeeded; blockIndex++) {
    let u: Uint8Array = new Uint8Array(hmacSha256(passwordBytes, concat(salt, int32BE(blockIndex))));
    let block = new Uint8Array(u);

    for (let i = 1; i < iterations; i++) {
      u = new Uint8Array(hmacSha256(passwordBytes, u));
